<template>
  <div class="subject">
      ...
      <div class="card-item__wrap" v-for="item in cardData" :key="item.id">
        <el-card class="card-item">
          ...
          <div class="card-item__operate">
            ...
            <label>
              <el-button type="text" class="button" @click="setSubject(item)">设置</el-button>
    ...
    <el-drawer
      ...
      :title="currentSubjectData.id ? '编辑' : '新增'"
      direction="rtl"
      size="650px"
      @close="resetData"
    >
...
export default {
  ...
  methods: {
    ...
    async setSubject(data) {
      this.currentSubjectData = Object.assign({}, data)
    ...
    saveForm() {
      ...
      form.validate(valid => {
        if (!valid) return
        let id = this.currentSubjectData.id
        let request = id ? editSubject(id, params) : newSubject(params)
